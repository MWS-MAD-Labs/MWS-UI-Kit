  ...
  return (
    <div className="min-h-screen bg-surface-base text-primary">
      ...
      {mobileOpen ? (
        ...
        >
          <button
            className={cx("absolute inset-0", overlayBackdropClassName)}
            type="button"
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      <div className="flex size-11 shrink-0 items-center justify-center radius-lg bg-brand-primary text-inverse shadow-brand">
        {logo ?? (
          <span className="heading-font text-sm font-extrabold">MWS</span>
        )}
      </div>
      <div className="min-w-0">
        <p className="heading-font truncate text-sm font-extrabold text-brand">
          {appName}
        </p>
        {appDescription ? (
          <p className="truncate text-xs text-tertiary">{appDescription}</p>
        ) : null}
      </div>
    </div>
  );
...
function Sidebar({
  ...
  return (
    <div className={cx("flex min-h-0 flex-1 flex-col", className)}>
      ...
      {footer ? (
        <div className="border-t border-subtle p-4">{footer}</div>
      ) : null}
    </div>
  );
}
...
function SidebarItem({
  ...
  const content = (
    <>
      {item.icon ? (
        <span className="shrink-0 text-tertiary group-[.is-current]:text-brand">
          {item.icon}
        </span>
      ) : null}
      ...
      {hasChildren ? (
        <ChevronDown
          className={cx("size-4 transition", expanded && "rotate-180")}
          aria-hidden="true"
        />
      ) : null}
    </>
        ...
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              {item.href && !isLast ? (
                <a
                  className="focus-ring radius-sm font-semibold text-link hover:text-[var(--mws-color-text-link-hover)]"
                  href={item.href}
                >
                  {item.label}
                </a>
                ...
                <span
                  className={cx(isLast && "font-semibold text-primary")}
    ...
    <div className={cx("relative", className)}>
      ...
      >
        ...
        <span className="hidden min-w-0 sm:block">
          <span className="heading-font block truncate text-sm font-bold text-primary">
            {name}
          </span>
          {role ? (
            <span className="block truncate text-xs text-tertiary">{role}</span>
          ) : null}
        </span>
        <ChevronDown
          className={cx(
      ...
      {open ? (
        <div
          id={menuId}
          role="menu"
          className="absolute right-0 mt-2 w-56 overflow-hidden radius-lg border border-subtle bg-surface-elevated p-2 shadow-lg"
        >
          {items.map((item) => {
            const classes = cx(
    ...
    <div className="relative">
      ...
      {open ? (
        <Card
          id={menuId}
          role="menu"
          className="absolute right-0 mt-2 w-80 p-3 shadow-lg"
          padding="compact"
        >
          <p className="heading-font px-2 pb-2 text-sm font-bold text-primary">
            MWS applications
          </p>
          <div className="grid gap-1">
            {apps.map((app) => (
              <a
                ...
                onClick={() => setOpen(false)}
              >
                <span className="flex size-10 shrink-0 items-center justify-center radius-lg bg-surface-base text-brand">
                  {app.icon ?? <Grid3X3 className="size-4" />}
                </span>
                <span>
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unreadCount ? (
          <span
            className="absolute right-1.5 top-1.5 size-2.5 radius-full bg-brand-gold"
            aria-hidden="true"
          />
        ) : null}
      </button>
      {open ? (
        <Card
          id={panelId}
          className="absolute right-0 mt-2 w-[min(24rem,calc(100vw-2rem))] p-0 shadow-lg"
          padding="none"
        >
          <div className="flex items-center justify-between border-b border-subtle p-4">
            <div>
            ...
            {notifications.length ? (
              notifications.map((notification) => {
                ...
                return notification.href ? (
                  <a
                    key={notification.id}
                    className="focus-ring block radius-lg"
              ...
              <div className="p-6 text-center">
                ...
                <p className="mt-1 text-sm text-tertiary">
                  Updates and review requests will appear here.
                </p>
              </div>
            )}
          </div>
